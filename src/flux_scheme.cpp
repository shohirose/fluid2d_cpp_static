...
void Fluid2d<Ni, Nj, Nb>::calc_flux_conv(const double& rho, const double& u,
                                         const double& v, const double& e,
                                         const double& ixS, const double& iyS,
                                         double (&Fc)[Nf]) const {
  // preperation
...
template <int Ni, int Nj, int Nb>
void Fluid2d<Ni, Nj, Nb>::roe_average(const double& rhoL, const double& uL,
                                      const double& vL, const double& eL,
                                      const double& rhoR, const double& uR,
                                      const double& vR, const double& eR,
                                      double& rhoA, double& uA, double& vA,
                                      double& eA) const {
  // preperation
...
template <int Ni, int Nj, int Nb>
void Fluid2d<Ni, Nj, Nb>::roe_fds(const double& rhoL, const double& uL,
                                  const double& vL, const double& eL,
                                  const double& rhoR, const double& uR,
                                  const double& vR, const double& eR,
                                  const double& ixS, const double& iyS,
                                  const double& S, double (&Fc)[Nf]) const {
  // preperation
  ...
  for (int i = 0; i < Nf; ++i) {
    ...
    for (int j = 0; j < Nf; ++j) {
      for (int k = 0; k < Nf; ++k) {
        Fc[i] -= R[i][j] * std::abs(LAM[j]) * Rinv[j][k] * (QR[k] - QL[k]);
      }
    }
    Fc[i] *= 0.5;
  }
