...
// Contents:
// + declaration of virtual class Fluid2d, which diesn't have the definition of
// EoS
...
class Fluid2d {
 ...
 protected:
  ...
  double x[Ni][Nj];  // x-coordinate list of grids: (Ni,Nj) array, evaluated at
                     // (i,j)
  double y[Ni][Nj];  // y-coordinate list of grids: (Ni,Nj) array, evaluated at
                     // (i,j)
  ...
  double u[Ni]
          [Nj];  // x-component of velocity: (Ni,Nj) array, evaluated at (i,j)
  double v[Ni]
          [Nj];  // y-component of velocity: (Ni,Nj) array, evaluated at (i,j)
  double e[Ni][Nj];  // total energy density per volume: (Ni,Nj) array,
                     // evaluated at (i,j)
  ...
  virtual void calc_eigen(const double& rho, const double& u, const double& v,
                          ...
                          double (&LAM)[Nf], double (&R)[Nf][Nf],
                          double (&Rinv)[Nf][Nf]) const = 0;
  ...
  void calc_conserved(const double& rho, const double& u, const double& v,
                      const double& e, const double& S, double (&Q)[Nf]) const;

  // construct convection fux from basic var
  void calc_flux_conv(const double& rho, const double& u, const double& v,
                      const double& e, const double& ixS, const double& iyS,
                      double (&Fc)[Nf]) const;

  // construct basic var from conserved var
  void calc_basic(const double (&Q)[Nf], const double& S, double& rho,
                  double& u, double& v, double& e) const;
  ...
  void roe_average(const double& rhoL, const double& uL, const double& vL,
                   ...
                   const double& vR, const double& eR, double& rhoA, double& uA,
                   double& vA, double& eA) const;
  //
  ...
  void reconstruction_basic_muscl3(
      const double& rho0, const double& rho1, const double& rho2,
      const double& rho3, const double& u0, const double& u1, const double& u2,
      const double& u3, const double& v0, const double& v1, const double& v2,
      const double& v3, const double& e0, const double& e1, const double& e2,
      const double& e3, double& rhoL, double& uL, double& vL, double& eL,
      double& rhoR, double& uR, double& vR, double& eR) const;
  void reconstruction_basic_mp5(
      const double& rho0, const double& rho1, const double& rho2,
      const double& rho3, const double& rho4, const double& rho5,
      const double& u0, const double& u1, const double& u2, const double& u3,
      const double& u4, const double& u5, const double& v0, const double& v1,
      const double& v2, const double& v3, const double& v4, const double& v5,
      const double& e0, const double& e1, const double& e2, const double& e3,
      const double& e4, const double& e5, double& rhoL, double& uL, double& vL,
      double& eL, double& rhoR, double& uR, double& vR, double& eR) const;
 ...
 public:
  //-----------------------Constructor---------------------------
  explicit Fluid2d(const std::string& dir_o, const std::string& f_coordinate,
                   const std::string& f_settings)
  ...
  void march_ssprk3(const double& dt, const std::string& bc_type,
                    const std::string& reconstruction = "MP5_basic",
                    const std::string& flux_scheme = "Roe_FDS");
  ...
  void output_basic(int tstep, int iter, float cpu_time,
                    float rest_time = 0.0f) const;
  ...
  int get_tstep() const { return tstep; };
...
class IdealGas2d : public Fluid2d<Ni, Nj, Nb> {
 protected:
  ...
  // calc pressure from density, velocity, total energy per volume
  double calc_p(const double rho, const double u, const double v,
                const double e) const override {
    return (gamma - 1.0) * (e - 0.5 * rho * (u * u + v * v));
  };

  // calc temperature from density, velocity, total energy per volume
  double calc_T(const double rho, const double u, const double v,
                const double e) const override {
    return e / rho - 0.5 * (u * u + v * v);
  };

  // calc sound-speed from density, velocity, total energy per volume
  double calc_cs(const double rho, const double u, const double v,
                 const double e) const override {
  ...
  // calc total energy per volume
  // from density, velocity, total specific enthalpy
  double calc_e(const double rho, const double u, const double v,
  ...
  // calc total energy per volume from density, velocity, pressure
  double calc_e_wp(const double rho, const double u, const double v,
  ...
  void calc_eigen(const double& rho, const double& u, const double& v,
                  const double& e, const double& ix, const double& iy,
                  double (&LAM)[Nf], double (&R)[Nf][Nf],
                  double (&Rinv)[Nf][Nf]) const override;

 public:
  //----------------------------Constructor--------------------------
  explicit IdealGas2d(const std::string& dir_o, const std::string& f_coordinate,
                      const std::string& f_settings, const double gamma = 1.4)
      : Fluid2d<Ni, Nj, Nb>(dir_o, f_coordinate, f_settings), gamma(gamma){};
