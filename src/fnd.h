...
class Fnd {
 private:
  ...
  static double minmod4(const double x1, const double x2, const double x3,
                        const double x4) {
    return 0.5 * (sign(x1) + sign(x2)) *
           std::abs(0.5 * (sign(x1) + sign(x3)) * 0.5 * (sign(x1) + sign(x4))) *
           std::min({std::abs(x1), std::abs(x2), std::abs(x3), std::abs(x4)});
  };

  // median for MP5
  static double median(const double x, const double y, const double z) {
 ...
 public:
  ...
  static void MUSCL3(const double& qm, const double& q, const double& qp,
                     const double& q2p, double& qL, double& qR);

  // MP5: alp=2 (Suresh & Huynh, 1997)
  // evaluating L/R at cell-boundary between q and qp
  static void MP5_sub(const double& q2m, const double& qm, const double& q,
                      const double& qp, const double& q2p, double& qL);

  static void MP5(const double& q2m, const double& qm, const double& q,
                  const double& qp, const double& q2p, const double q3p,
                  double& qL, double& qR) {
